...
<html lang="en">
    ...
    <body data-background-color="bg3">
                ...
                <div class="content">
                    <div class="page-inner">
						...
						<div class="row">
                            ...
                            <div class="col-sm-2">
                            	<button type="button" id="submit" class="btn btn-block btn-primary">Selesaikan pengembalian</button>
        ...
        <script>
			...
			$(document).ready(function() {
				const tabelItem = $('#item').DataTable({
					...
					ajax: {
						...
						type: 'POST',
						data: {
							'id_lab': function(){return $("#id_lab").val();},
			...
			document.getElementById('kode').addEventListener('keydown', function (event) {
				if (event.key === 'Enter') {
					event.preventDefault(); // Hindari submit form jika ada

					const nilaiInput = this.value.trim();
					if (!nilaiInput) return; // Jangan lanjut jika kosong

					// Ambil semua checkbox dari kedua grup
					const semuaCheckbox = document.querySelectorAll('input[name="kodeItem[]"], input[name="kodeKotak[]"]');

					semuaCheckbox.forEach(checkbox => {
						if (checkbox.value === nilaiInput) {
